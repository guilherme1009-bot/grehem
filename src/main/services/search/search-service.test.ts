...
describe('SearchService', () => {
  ...
  it('allows HTTP, HTTPS, and Grehem URLs', () => { const value = setup(); const service = new SearchService(value.engines, value.profile.id); expect(service.resolve('http://example.com')).toBe('http://example.com'); expect(service.resolve('https://example.com/a')).toBe('https://example.com/a'); expect(service.resolve('grehem://newtab')).toBe('grehem://newtab'); value.database.close() })
  it('rejects explicit unsafe or unknown protocols', () => { const value = setup(); const service = new SearchService(value.engines, value.profile.id); for (const input of ['javascript:alert(1)', 'file:///etc/passwd', 'data:text/html,test', 'vbscript:msgbox(1)', 'chrome://settings', 'custom:thing']) expect(() => service.resolve(input)).toThrow(/Protocolo/); value.database.close() })

...
export function isNavigableUrl(value: string): boolean { if (/\s/.test(value)) return false; try { const parsed = new URL(value); return allowedProtocols.has(parsed.protocol) && Boolean(parsed.hostname || parsed.protocol === 'grehem:') } catch { return false } }
...
export class SearchService {
  ...
  resolve(input: string): string { const value = input.trim(); if (!value) return 'grehem://newtab'; if (/^[a-z][a-z\d+.-]*:/i.test(value)) { if (isNavigableUrl(value)) return value; if (/^grehem:/i.test(value)) throw new Error('URL interna inválida'); } if (looksLikeHostname(value)) return `https://${value}`; const engine = this.engines.getDefault(this.profileId); if (!engine || !engine.search_url.includes('%s')) throw new Error('Mecanismo de pesquisa inválido'); return engine.search_url.replaceAll('%s', encodeURIComponent(value)) }
}

...
export class HistoryRepository {
  ...
  add(profileId: number, url: string, title = '', favicon: string | null = null): HistoryEntry { if (url.startsWith('grehem://')) throw new Error('internal URLs are not stored in history'); const safeTitle = title === 'Carregando…' ? '' : title; const last = this.db.prepare('SELECT * FROM history WHERE profile_id = ? ORDER BY visited_at DESC, id DESC LIMIT 1').get(profileId) as HistoryEntry | undefined; let id: number | bigint; if (last?.url === url) { this.db.prepare('UPDATE history SET title = ?, favicon = ?, visited_at = CURRENT_TIMESTAMP WHERE id = ?').run(safeTitle, favicon, last.id); id = last.id } else { id = this.db.prepare('INSERT INTO history (profile_id, url, title, favicon) VALUES (?, ?, ?, ?)').run(profileId, url, safeTitle, favicon).lastInsertRowid } return this.db.prepare('SELECT * FROM history WHERE id = ?').get(id) as HistoryEntry }
